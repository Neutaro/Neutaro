...
ifeq (cleveldb,$(findstring cleveldb,$(NEUTARO_BUILD_OPTIONS)))
  build_tags += gcc
else ifeq (rocksdb,$(findstring rocksdb,$(NEUTARO_BUILD_OPTIONS)))
  build_tags += gcc
endif
...
ifeq (,$(findstring nostrip,$(NEUTARO_BUILD_OPTIONS)))
  ldflags += -w -s
endif
...
ifneq ($(GO_SYSTEM_VERSION), $(REQUIRE_GO_VERSION))
	@echo "ERROR: Go $(REQUIRE_GO_VERSION) is required for $(VERSION) version of Neutaro. Currently Go $(GO_SYSTEM_VERSION) is being used."
	exit 1
endif
...
SIMAPP = github.com/Neutaro/Neutaro-Chain/app/simulation
