...
$BINARY init test --home $CHAIN_DIR --chain-id=$CHAIN_ID --default-denom uneutaro
...
$BINARY genesis add-genesis-account $($BINARY --home $CHAIN_DIR keys show alice --keyring-backend test -a) 100000000000uneutaro  --home $CHAIN_DIR
$BINARY genesis add-genesis-account $($BINARY --home $CHAIN_DIR keys show bob --keyring-backend test -a) 100000000000uneutaro  --home $CHAIN_DIR
$BINARY genesis add-genesis-account $($BINARY --home $CHAIN_DIR keys show validator --keyring-backend test -a) 100000000000uneutaro  --home $CHAIN_DIR

$BINARY genesis gentx validator 7000000000uneutaro --home $CHAIN_DIR --chain-id $CHAIN_ID --keyring-backend test
$BINARY genesis collect-gentxs --home $CHAIN_DIR

sed -i -e 's/stake/uneutaro/g' $CHAIN_DIR/config/genesis.json
...
sed -i.bak -e "s/^minimum-gas-prices *=.*/minimum-gas-prices = \"0.025uneutaro\"/" $CHAIN_DIR/config/app.toml
