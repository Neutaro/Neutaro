#!/bin/bash

# Colors for output
RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[0;33m'
NC='\033[0m' # No Color

echo -e "${GREEN}Starting Neutaro Validator Auto-Setup Script${NC}"

# Detect current user and home directory
if [ "$EUID" -eq 0 ]; then
    CURRENT_USER=${SUDO_USER:-root}
    HOME_DIR=$(eval echo "~$CURRENT_USER")
else
    CURRENT_USER=$(whoami)
    HOME_DIR=$HOME
fi

# Prompt for Moniker Name
read -p "Enter your moniker name: " MONIKER

# Set default seeds and pruning options
DEFAULT_SEEDS="0e24a596dc34e7063ec2938baf05d09b374709e6@109.199.106.233:26656"
read -p "Enter seeds (default: $DEFAULT_SEEDS): " SEEDS
SEEDS=${SEEDS:-$DEFAULT_SEEDS}

PRUNING="custom"
PRUNING_KEEP_RECENT="100"
PRUNING_INTERVAL="19"

echo -e "${GREEN}Pruning is set to custom with the following settings:${NC}"
echo -e "${GREEN}Keep recent: $PRUNING_KEEP_RECENT, Interval: $PRUNING_INTERVAL${NC}"

# Inform about persistent peers
echo -e "${GREEN}Important: If you face sync issues due to limited seeds, you may need to add more persistent peers.${NC}"
echo -e "${GREEN}Also, opening port 26656 for TCP connections is recommended to share your node with others and improve the network.${NC}"

# Prompt for persistent peers (optional)
read -p "Enter persistent peers (optional, leave blank to skip): " PERSISTENT_PEERS

# Update and install dependencies
echo -e "${GREEN}Updating and installing dependencies...${NC}"
sudo apt update && sudo apt upgrade -y
sudo apt install curl tar wget clang pkg-config libssl-dev jq build-essential bsdmainutils git make ncdu gcc chrony liblz4-tool pv -y
if [ $? -ne 0 ]; then
    echo -e "${RED}Failed to install dependencies. Exiting.${NC}"
    exit 1
fi

# Install Go
GO_VERSION="1.22.2"
echo -e "${GREEN}Installing Go $GO_VERSION...${NC}"
cd $HOME_DIR
wget "https://dl.google.com/go/go$GO_VERSION.linux-amd64.tar.gz"
sudo rm -rf /usr/local/go
sudo tar -C /usr/local -xzf "go$GO_VERSION.linux-amd64.tar.gz"
rm "go$GO_VERSION.linux-amd64.tar.gz"
echo "export PATH=$PATH:/usr/local/go/bin:$HOME_DIR/go/bin" >> $HOME_DIR/.bash_profile
source $HOME_DIR/.bash_profile
go version >/dev/null 2>&1

if ! command -v go &> /dev/null || [[ "$(go version)" != *"$GO_VERSION"* ]]; then
    echo -e "${RED}Go installation failed or incorrect version installed. Exiting.${NC}"
    exit 1
fi

# Clone and build Neutaro
echo -e "${GREEN}Cloning and building Neutaro...${NC}"
cd $HOME_DIR
git clone https://github.com/Neutaro/Neutaro || { echo -e "${RED}Failed to clone Neutaro repository. Exiting.${NC}"; exit 1; }
cd Neutaro
make build || { echo -e "${RED}Failed to build Neutaro. Exiting.${NC}"; exit 1; }

# Install Cosmovisor
echo -e "${GREEN}Setting up Cosmovisor...${NC}"
mkdir -p $HOME_DIR/.Neutaro/cosmovisor/genesis/bin
mv $HOME_DIR/Neutaro/build/Neutaro $HOME_DIR/.Neutaro/cosmovisor/genesis/bin/
sudo ln -sfn $HOME_DIR/.Neutaro/cosmovisor/genesis $HOME_DIR/.Neutaro/cosmovisor/current
sudo ln -sfn $HOME_DIR/.Neutaro/cosmovisor/current/bin/Neutaro /usr/local/bin/Neutaro
cd $HOME_DIR/Neutaro/
go install cosmossdk.io/tools/cosmovisor/cmd/cosmovisor@v1.4.0 || { echo -e "${RED}Failed to install Cosmovisor. Exiting.${NC}"; exit 1; }

# Initialize and configure the node
echo -e "${GREEN}Configuring your Neutaro node...${NC}"
Neutaro init $MONIKER --chain-id Neutaro-1 || { echo -e "${RED}Failed to initialize the node. Exiting.${NC}"; exit 1; }
Neutaro config chain-id Neutaro-1
Neutaro config keyring-backend os
curl http://154.26.153.186/genesis.json > $HOME_DIR/.Neutaro/config/genesis.json

# Configure app.toml
echo -e "${GREEN}Setting gas prices and pruning options...${NC}"
sed -i "s/^minimum-gas-prices *=.*/minimum-gas-prices = \"0uneutaro\"/" $HOME_DIR/.Neutaro/config/app.toml
sed -i "s/^pruning *=.*/pruning = \"$PRUNING\"/" $HOME_DIR/.Neutaro/config/app.toml
sed -i "s/^pruning-keep-recent *=.*/pruning-keep-recent = \"$PRUNING_KEEP_RECENT\"/" $HOME_DIR/.Neutaro/config/app.toml
sed -i "s/^pruning-interval *=.*/pruning-interval = \"$PRUNING_INTERVAL\"/" $HOME_DIR/.Neutaro/config/app.toml

# Configure config.toml
sed -i "s/^seeds *=.*/seeds = \"$SEEDS\"/" $HOME_DIR/.Neutaro/config/config.toml
if [ -n "$PERSISTENT_PEERS" ]; then
    sed -i "s/^persistent_peers *=.*/persistent_peers = \"$PERSISTENT_PEERS\"/" $HOME_DIR/.Neutaro/config/config.toml
fi

# Open port 26656
sudo ufw allow 26656/tcp

# Download and apply the new snapshot
echo -e "${GREEN}Downloading and applying the blockchain snapshot...${NC}"
cd $HOME_DIR
wget https://snapshot.neutaro.tech/latest.tar.lz4
mkdir -p $HOME_DIR/.Neutaro/data
pv latest.tar.lz4 | lz4 -d - | tar xvf - -C $HOME_DIR/.Neutaro/data --strip-components=1
rm latest.tar.lz4

# Create missing priv_validator_state.json
echo -e "${GREEN}Creating priv_validator_state.json...${NC}"
cat <<EOF > $HOME_DIR/.Neutaro/data/priv_validator_state.json
{
  "height": "0",
  "round": 0,
  "step": 0
}
EOF

# Fix permissions
sudo chown -R $CURRENT_USER:$CURRENT_USER $HOME_DIR/.Neutaro

# Create systemd service for Neutaro
echo -e "${GREEN}Creating systemd service for Neutaro...${NC}"
sudo tee /etc/systemd/system/Neutaro.service > /dev/null << EOF
[Unit]
Description=Neutaro Node Service
After=network-online.target

[Service]
User=$CURRENT_USER
ExecStart=/usr/local/bin/cosmovisor run start
Restart=on-failure
RestartSec=10
LimitNOFILE=65535
Environment="DAEMON_HOME=$HOME_DIR/.Neutaro"
Environment="DAEMON_NAME=Neutaro"
Environment="UNSAFE_SKIP_BACKUP=true"

[Install]
WantedBy=multi-user.target
EOF

# Enable and start the service
echo -e "${GREEN}Starting Neutaro service...${NC}"
sudo systemctl daemon-reload
sudo systemctl enable Neutaro
sudo systemctl restart Neutaro || { echo -e "${RED}Failed to start Neutaro service. Exiting.${NC}"; exit 1; }

# Final instructions
echo -e "${GREEN}Setup complete!${NC}"
echo -e "Check sync status with: Neutaro status 2>&1 | jq .SyncInfo"
echo -e "Monitor logs with: sudo journalctl -fu Neutaro -o cat"
